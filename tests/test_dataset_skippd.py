...
@pytest.fixture
def skippd_testset():
    return SKIPPD(download=False, train=False)


@pytest.fixture
def skippd_transformed_testset():
    transform = transforms.ToTensor()
    return SKIPPD(download=False, train=False, transform=transform)
...
def test_skippd_testset_dataloader(skippd_transformed_testset):
    ...
    dataloader = torch.utils.data.DataLoader(
        skippd_transformed_testset, batch_size=batch_size, shuffle=False, num_workers=0
    )
